/*
	먼저 말씀드리자면 풀지 못했습니다.
	다만 어떤 사고 과정을 거쳐 어디까지 해결을 했는지라도 보여드리기 위해 제출합니다.
	코드 뒤에 사고 과정을 서술해 두었습니다.
	괜찮으시다면 봐주세요.
	감사합니다.
*/
...
func solution(w,h int) int {
	...
	if w == 1 {
		return (h+1) % 1000000007
	}
	
	if w == 2 {
		
		ans := 6 * int(math.Pow(2, float64(h))) - 2*h - 5
	...
	for i := 0; i < h; i++ {
		memo[2][i] = 6 * int(math.Pow(2, float64(i))) - 2*i - 5
	}
...
각 base block의 넓이 = 1, 2... 의 경우를 메모이제이션해두면 연산도 많지 않게 될 것 같다.
...
w = 3 인 경우부터는 브릿지되는 경우가 있어서 더 복잡한데 아마 이 때부턴 일반화를 해야하거나

재귀적으로 어떻게 쪼갤 수 있는지 봐야할 것 같다.
...
w = 4 인 경우부터는 아마 w=3, 2, 1인 경우의 조합으로 나타날 듯..
*/